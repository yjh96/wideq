...
V2_CLIENT_ID = \
    '65260af7e8e6547b51fdccf930097c51eb9885a508d3fddfa9ee6cdec22ae1bd'
V2_MESSAGE_ID = 'wideq'
V2_SVC_PHASE = 'OP'
V2_APP_LEVEL = 'PRD'
V2_APP_OS = 'LINUX'
...
def thinq2_headers(country: str,
                   language: str,
...
def thinq2_get(url, access_token=None, user_number=None, headers={},
               country="US", language="en-US") -> Dict[str, Any]:
    """Get request to the thinq2 API"""
    res = requests.get(url, headers=thinq2_headers(
        access_token=access_token, user_number=user_number,
...
def parse_oauth_callback(url) -> Tuple[str, str, str]:
    """Parse the URL to which an OAuth login redirected to obtain
    the backend url, a token and the user ID.
    """
    params = parse_qs(urlparse(url).query)
    return (params['oauth2_backend_url'][0],
            params['code'][0],
...
class Gateway(object):
    ...
    def discover(cls, country, language) -> 'Gateway':
        # ### FIXME: remove either auth_base or oauth_root, we set them to the same value,
        # only one is needed with the new API
        """Load information about the host to use for API interaction.
...
class Auth(object):
    def __init__(self, gateway: str, oauth_url: str, access_token: str,
                 refresh_token: str, user_number: str) -> None:
    ...
    def from_url(cls, gateway, url):
        ...
        return cls(gateway, oauth_url, access_token,
                   refresh_token, user_number)
    ...
    def refresh(self):
        ...
        new_access_token = refresh_auth(self.oauth_url,
                                        self.refresh_token)
        return Auth(self.gateway, self.oauth_url, new_access_token,
                    self.refresh_token, self.user_number)
    ...
    def load(cls, gateway, data):
        return cls(gateway, data['oauth_url'], data['access_token'],
                   data['refresh_token'], data['user_number'])
